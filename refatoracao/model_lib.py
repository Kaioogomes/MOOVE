...
class ModelSettings:
    def __init__(self, tipo, atividade, tam_janela, sensores, igualar_janelas, particao, normalizado):
        self.tipo = tipo
    ...
    def __eq__(self, settings):
        return(((self.tipo == settings.tipo) and
                (self.atividade == settings.atividade) and
                (self.tam_janela == settings.tam_janela) and
                (self.sensores == settings.sensores) and
                (self.igualar_janelas == settings.igualar_janelas) and
                (self.particao == settings.particao) and
                (self.normalizado == settings.normalizado)))

class ModeloMoove:
    def __init__(self, modelo, fator_norm, settings):
        self.modelo = modelo
        self.fator_norm = [np.transpose(fator_norm[0]), np.transpose(fator_norm[1])]
        self.settings = settings

    # def set_info(self, atividade, tam_janela, sensores, igualar_janelas, particao, normalizado):
    #     self.atividade = atividade
    #     self.tam_janela = tam_janela
    #     self.sensores = sensores
    #     self.igualar_janelas = igualar_janelas
...
def gerar_modelo_moove(dados_atividade, fator_norm, settings):
    dados_atividade = dados_atividade.sample(frac=1).reset_index(drop=True).iloc[:, 11:]
    ...
    modelo = ModeloMoove(classificador, fator_norm, settings)
...
def relatorio_moove(table, tam_janela, channel_comb,name_suffix="", tipo_arquivo=0, tipo_modelo=1, sensores=['LSK', 'RUL', 'Trunk'], exec_all=True, n_fold=5, igualar_janelas=True, normalizar=True):
    ...
    with open(nome_arquivo_out, 'w') as arquivo:
        ...
        for atividade in atividades:
            ...
            modelo_conf = ModelSettings(modelo, atividade, tam_janela, sensores, igualar_janelas, particao, normalizar)

            modelo_moove = gerar_modelo_moove(dados_atividade, fator_norm, modelo_conf)
            

            dir_modelos = "./modelos/"

            dump(modelo_moove, f"{dir_modelos}_{modelo}_{atividade}_{''.join(sensores)}_igJan={igualar_janelas}_{particao}_normalizar={normalizar}.joblib")
